...
class LogSystem<T extends ILog & Comparable<T>>{
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logs -> (ArrayList<T>) logs.stream().filter(log->log.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<T>secondLogProcessor = logs -> (ArrayList<T>) logs.stream().filter(log->log.getMessage().length()<100).collect(Collectors.toList());

        LogProcessor<T> thirdLogProcessor =logs -> (ArrayList<T>) logs.stream().sorted().collect(Collectors.toList());
