package Napredno.K2.GenericCollection;
...
class GenericCollection<T extends  Comparable<T> & IHasTimestamp>{
    Map<String,Set<T>> byCategory;
    Map<String,Set<T>> byTimeStamp;
    public GenericCollection(){
        byCategory=new HashMap<>() ;
        byTimeStamp=new TreeMap<>();
    }

    public void addGenericItem(String category, T integerElement) {
        byCategory.putIfAbsent(category,new TreeSet<>(Comparator.reverseOrder()));
        byCategory.get(category).add(integerElement);

        byTimeStamp.putIfAbsent(getMonthandDay(integerElement.getTimestamp()),new TreeSet<>(Comparator.reverseOrder()));
        byTimeStamp.get(getMonthandDay(integerElement.getTimestamp())).add(integerElement);
    }
    public Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
        return byCategory.values().stream().flatMap(Set::stream).filter(t -> (t.getTimestamp().isAfter(from) && t.getTimestamp().isBefore(to)) || t.getTimestamp()==from || t.getTimestamp()==to).sorted(Comparator.reverseOrder()).collect(Collectors.toList());

    }

    public  Collection<T> itemsFromCategories(List<String> categories) {
        return byCategory.entrySet().stream().filter(stringSetEntry -> categories.contains(stringSetEntry.getKey())).map(Map.Entry::getValue).flatMap(Set::stream).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    }
    ...
    private String getMonthandDay(LocalDateTime dateTime){
        return String.format("%02d-%02d",dateTime.getMonth().getValue(),dateTime.getDayOfMonth());
    }
}
